...
function LottieConverter(opts) {
    ...
    async function saveAs(args) {
        const { lottiePath, outputPath, loop = '0', scale = 1 } = args;
        const fsDataStr = fs.readFileSync(
            lottiePath,
            'utf-8'
        )
        let fsData = JSON.parse(fsDataStr);
        return await dataSaveAs({ lottieData: fsData, outputPath: outputPath, loop: loop, scale });
    }
    ...
    async function dataSaveAs(args) {
        ...
        const { lottieData, outputPath, loop = '0', scale = 1 } = args;
